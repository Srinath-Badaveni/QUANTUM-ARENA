    ...
    <div className="treasurer-container" style={{maxWidth: '1000px', background: '#fff', color: '#000', minHeight: '100vh', padding: '20px'}}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <div className="no-print" style={{ display: 'flex', gap: '10px' }}>
          ...
          />
          <button onClick={() => window.print()} style={{ padding: '8px 16px', background: '#000', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
            PRINT LIST
          </button>
          <button onClick={() => {
            const verifyHash = import.meta.env.VITE_VERIFY_HASH || '#verify';
            window.location.hash = verifyHash;
          }} style={{ padding: '8px 16px', background: '#ddd', color: '#000', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
            BACK
          </button>
        </div>
      </div>
